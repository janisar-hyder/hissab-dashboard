...
@Component({
  selector: 'app-debit-notes-new',
  standalone: true,
  imports: [CommonModule, FormsModule, ButtonComponent, BreadcrumbsComponent, AttachmentsModal, CustomSelectComponent],
  templateUrl: './debit-notes-new.html',
  styleUrl: './debit-notes-new.scss',
})
export class DebitNotesNew implements OnInit {
  ...
  onItemSelect(item: DebitNoteItem): void {
  ...
  get totalDiscount(): number {
    if (this.debitNoteData.discountAt === 'Transaction Level') {
      const discount = Number(this.debitNoteData.transactionDiscount) || 0;
      if (this.debitNoteData.transactionDiscountType === '%') {
  ...
  updateAmount(item: DebitNoteItem): void {
    const rate = Number(item.rate) || 0;
    const qty = Number(item.qty) || 0;
    const discount = this.debitNoteData.discountAt === 'Line Item Level' ? (Number(item.discount) || 0) : 0;
    const vat = Number(item.vat) || 0;
