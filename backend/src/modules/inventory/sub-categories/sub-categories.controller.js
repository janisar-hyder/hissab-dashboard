...
/**
 * @desc    Update status for multiple sub-categories
 * @route   PATCH /api/v1/inventory/sub-categories/bulk-status
 */
exports.updateBulkStatus = async (req, res, next) => {
  try {
    ...
    const updateResult = await prisma.inventorySubCategory.updateMany({
      where: {
        client_id: clientId,
    ...
    res.status(200).json({
      success: true,
      message: `Successfully updated status to ${status} for ${updateResult.count} sub-categories`
    });
  } catch (err) {
    next(err);
  }
